...
it('register a new order', function () {
    ...
    $endpoint = (string) str(config('paymob.payments.payment_methods.card.iframe_link'))->beforeLast('/');
...
{
    $config = new PaymobConfig(config('paymob.payments'));
...
{
    Http::fake([
        config('paymob.payouts.endpoints.authentication_request') => Http::response(
            json_decode(stripslashes('{"access_token": "fYfXmY5pp3zaFHYKxhGrFmOK7o9RNU", "expires_in": 3600, "token_type": "Bearer", "scope": "read write groups", "refresh_token": "mJBii6HQcg5RjOcdMeKsropXYmkTha"}'), true),
            200
        ),
        config('paymob.payouts.endpoints.disburse') => Http::response(
            json_decode(stripslashes('{"transaction_id":"1e4f0d8d-fb21-4042-bdc3-7568edb9b64d","issuer":"vodafone","msisdn":"01023456789","amount":100.0,"full_name":"","disbursement_status":"successful","status_code":"200","status_description":"","aman_cashing_details":null,"client_transaction_reference":null,"created_at":"2022-08-19 18:50:07.141505","updated_at":"2022-08-19 18:50:07.168161"}'), true),
            200
        ),
        config('paymob.payments.endpoints.pay') => Http::response(
        ...
        config('paymob.payments.endpoints.authentication_request') => Http::response([
            'token' => getApiKey(),
        ], 200),
        config('paymob.payments.endpoints.order_registration') => Http::response([
            'id' => 61747044,
        ...
        config('paymob.payments.endpoints.payment_keys') => Http::response(
            [
                'id' => 61747044,
                'created_at' => '2022-08-10T14:47:40.146080',
